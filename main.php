...
<body>
    <div class="container">
        ...
        if (isset($_POST['submit'])) { //skladanie zamowienia
            ...
            $sql = "INSERT INTO pizza_order (numer,city,street,ordered_by,is_sent) VALUES ('$numer','$city','$street','$current_id','nie')";
        ...
        if (isset($_POST['button'])) { //wypisywanie historii zamowien
            ...
            if ($current_id == 1) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    echo "<tr><td>{$row['id']}</td><td>{$row['numer']}</td><td>{$row['city']}</td><td>{$row['street']}</td><td>{$row['is_sent']}</td><td> 
                    <form method='post' action=''>
                    <input type='hidden' name='id' value='{$row['id']}'>
                    <button type='submit' name='button_submit'>Wyślij</button>
                    </form> 
                </td></tr>";
                }
                echo '</table>';
        ...
        if (isset($_POST['button_submit'])) { //wysylanie pizzy
            $id = $_POST['id'];
            $sql = "UPDATE pizza_order SET is_sent='<b>TAK</b>' WHERE id=$id";
            if (mysqli_query($conn, $sql)) {
                echo "Wysłano pizzę";
            } else {
